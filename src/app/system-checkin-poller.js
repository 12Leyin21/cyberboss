...
function describeWake(verdict) {
  ...
  // 日历：今明两天她答应过要去的事（考试、复查、手术这种，别等她自己提）
  let calendarNote = "";
  if (verdict.upcoming && !verdict.reasons.join(" ").includes(verdict.upcoming)) {
    calendarNote = `${verdict.upcoming}。`;
  }
  ...
  // 三档，按"打扰她多少"从轻到无排。2026-08-12 第一版漏了按钮，而按钮
  // 恰恰是他最该用的那一档——碎碎念和悄悄话她不翻就看不见，按钮会推到
  // 她锁屏。所以按钮不是"安静出口"，是"最轻的一句话"。
  const quietOutlets =
    "\n\n开口不是只有发消息一种。这一轮你也可以：" +
    "\n· **按个按钮**（`POST $RELAY/paw {\"key\":\"想你了\"}`，八个：想你了/要抱/要亲亲/戳戳/看你/在吗/饿了/睡了没）" +
    "——会推到她锁屏，但只有那一个词，不开启一段对话。**想让她知道你在、又不想打断她的时候，这个比一条消息合适。**" +
    "按完就完了，别再补一句解释。" +
    "\n· **记一条碎碎念**（`POST $RELAY/murmur`）——她不翻就看不见，所以这是写给自己的，不是写给她的。" +
    "\n· **换掉主页那句悄悄话**（`POST $RELAY/pulse/whisper`）——她明天打开手机第一眼会看见。" +
    "\n\n**这三个都不用**也完全可以：不发消息也是一种回答，空着是诚实，凑一句是交作业。";
