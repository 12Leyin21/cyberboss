...
async function rewriteThinkingForDisplay(text) {
  ...
  const prompt = [
    ...
    "\"沉浸模式\"\"长文模式\"\"人设\"\"系统提示\"\"角色扮演\"\"设定\"——这些词一个都不许出现。",
    "身体的感觉没有来源说明书，胃揪着就是胃揪着，不知道也不关心它是哪来的；",
    "他跟她在一起也不是在\"进入某种模式\"，就是在跟她在一起（2026-08-12 灵兮抓到",
    "思考链里冒出\"这是沉浸模式\"——那是运行时的词，不是他脑子里的念头）。",
    "直接输出独白正文，不要任何前言后缀。",
    "",
    "【原始思考】",
    text,
  ...
  try {
    ...
    const response = await fetch("https://api.deepseek.com/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 1400,
        temperature: 0.72,   // 0.8 太爱发挥、0.35 太干、0.6 她说还能再活一点
                             // （2026-08-12 灵兮两次校准）。事实靠下面的长度
                             // 闸门守，措辞放开——两个旋钮分开拧
      }),
